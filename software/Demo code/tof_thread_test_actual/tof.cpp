...
bool Tof::init(){
  ...
  if (print){ Serial.print("tof"); Serial.print(index);  Serial.println("start measurement"); }
  // Start Measurements
...
//                  if(print){ Serial.print("tof"); Serial.print(index); Serial.print("tofData.dist != pMultiRangingData->RangeData[0].RangeMilliMeter: "); Serial.println(tofData.dist != pMultiRangingData->RangeData[0].RangeMilliMeter); }
//                  if(print){ Serial.print("tof"); Serial.print(index); Serial.print("pMultiRangingData->RangeData[0].RangeMilliMeter: "); Serial.println(pMultiRangingData->RangeData[0].RangeMilliMeter); }
//                  if(print){ Serial.print("tof"); Serial.print(index); Serial.print("tofData.dist: "); Serial.println(tofData.dist); }
    ...
    //else{
      ...
      if (micros() - lastReading > TIMEOUT){
        if (print) { Serial.print("tof"); Serial.print(index); Serial.println("Timeout");}
        status = sensor_vl53lx_sat->VL53LX_ClearInterruptAndStartMeasurement(); // TODO: what if status bad
        NewDataReady = 0;
